...
- **`cmd/`** — Cobra command definitions. All top-level commands (`review`, `comments`, `threads`) use the subcommand pattern. The `review` command is a container with subcommands: `start`, `add-comment`, `edit`, `edit-comment`, `delete-comment`, `submit`, `preview`, `view` (each in its own `review_*.go` file). `deps.go` holds a `apiClientFactory` function variable for dependency injection in tests.
